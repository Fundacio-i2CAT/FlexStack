...
class Test_if_ldm_3(unittest.TestCase):
    ...
    def test_check_permissions(self):
        ...
        self.assertEqual(self.if_ldm_3.check_permissions(access_permisions, data_object_id), False)

        access_permisions = [VAM]
        data_object_id = VAM
        self.assertEqual(self.if_ldm_3.check_permissions(access_permisions, data_object_id), True)
    ...
    def test_deregister_data_provider(self):
        ...
        self.assertEqual(
            self.if_ldm_3.deregister_data_provider(data_provider_incorrect).result.result,
            1,
        )
    ...
    def test_add_provider_data(self):
        ...
        data_provider = AddDataProviderReq(
            CAM,
            TimestampIts.initialize_with_timestamp_its(5000),
            Location.initializer(latitude=2.3, longitude=41.0),
            white_cam,
            TimeValidity(5),
        )

        self.assertIsInstance(self.if_ldm_3.add_provider_data(data_provider), AddDataProviderResp)
        self.assertEqual(self.if_ldm_3.add_provider_data(data_provider).data_object_id, "unsuccessful")
        ...
        self.assertIsInstance(self.if_ldm_3.add_provider_data(data_provider).data_object_id, int)
    ...
    def test_update_provider_data(self):
        ...
        self.ldm_service.ldm_maintenance.data_containers.exists = MagicMock(return_value=False)

        data_provider = UpdateDataProviderReq(35, DataContainer(35), None, None, white_cam, None)
        self.assertIsInstance(self.if_ldm_3.update_provider_data(data_provider), UpdateDataProviderResp)
        self.assertEqual(self.if_ldm_3.update_provider_data(data_provider).result.result, 1)

        self.ldm_service.ldm_maintenance.data_containers.exists = MagicMock(return_value=True)
        data_provider = UpdateDataProviderReq(2, DataContainer(1), None, None, white_cam, None)
        self.assertEqual(self.if_ldm_3.update_provider_data(data_provider).result.result, 0)
    ...
    def test_delete_provider_data(self):
        ...
        self.assertIsInstance(self.if_ldm_3.delete_provider_data(data_provider), DeleteDataProviderResp)
        self.assertEqual(
